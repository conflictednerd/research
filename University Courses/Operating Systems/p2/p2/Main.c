...
-----------------------------  Dining Philosophers  ---------------------------------
  ...
  class ForkMonitor
    ...
    fields
      ...
      conditions: array [5] of Condition  -- For each philosopher
      globalLock: Mutex
    methods
      Init ()
      Check (p: int)
  ...
  behavior ForkMonitor
    method Init ()
      ...
      conditions = new array of Condition {5 of new Condition}
    ...
    -- Checks if the p-th philosopher can eat. If he can, his status will be changed.
    method Check (p: int)
    ...
    method PickupForks (p: int)
      -- This method is called when philosopher 'p' is wants to eat.
        ...
        self.PrintAllStatus ()
        self.Check (p)
        -- A while loop is used instead of an if statement because the condition is Mesa-style
        while status[p] != EATING
          conditions[p].Wait(&globalLock)
          self.Check (p)
        endWhile
    ...
    method PutDownForks (p: int)
      ...
      status[p] = THINKING
      conditions[(p+1) % 5].Signal(&globalLock)
      conditions[(p-1) % 5].Signal(&globalLock)
