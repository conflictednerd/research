...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      -- First check if the currentThread already has the lock
      -- If the lock is available, give it to the current thread
      -- If not, add this thread to the waiting list and go to sleep
      ...
      -- First, checks if the current thread holds the lock.
      -- If waitingThreads is empty, release the lock by setting heldBy = null
      -- If waitingThreads is not empty, give the lock to a waiting thread by setting heldBy = t
      -- and wake up thread t that has aquired the lock.
      ...
      method Unlock ()
          ...
          oldIntStat = SetInterruptsTo(DISABLED)
          if heldBy != currentThread
            FatalError ("Illegal mutex unlock request")
          endIf
          t = waitingThreads.Remove()
          heldBy = t
          if t != null
      ...
      -- We must disable the interrupt because heldBy is a global variable and maybe changed during execution
