code Kernel
  ...
  -----------------------------  InitFirstProcess  ---------------------------------
  -- This function creates a new thread and lets it run the function "StartUserProcess ()"
  function InitFirstProcess ()
    var thread: ptr to Thread
    print ("Loading initial program...")
    nl ()
    thread = threadManager.GetANewThread ()
    thread.Init ("User Program")
    thread.Fork (StartUserProcess, 0)
  endFunction

  -----------------------------  StartUserProcess  ---------------------------------
  -- This function is called by a new thread and starts up a new user process.

  function StartUserProcess (argument: int)
    var
    ...
    -- First, we should create a new processControlBlock and connect it to the running thread
    pcb = processManager.GetANewProcess ()
    pcb.myThread = currentThread
    currentThread.myProcess = pcb

    -- Next, we must load the executable file
    openFile = fileManager.Open ("TestProgram1")
    initPC = openFile.LoadExecutable (& pcb.addrSpace)
    fileManager.Close (openFile)

    -- Finally, we should execute the loaded program and remove all the added variables from kernel stack
...
-----------------------------  HoareCondition  ---------------------------------
    -- This class can be used to implement monitors with hoare semantics
    -- A monitor is implemented via a Class that has several "entery methods" and "conditions".
    -- Each monitor has a lock(Binary semaphore) initialized to 1 associated with it just like before.
    -- In addition, each monitor class must have a GLOBAL integer field called "nextCount" and a semaphore called "nexSem"
    -- These must both be initialized to 0.
    -- Entry methods: 
      -- At the beginning of each entry method, monitors lock must be acquired : monitorLock.Down ()
      -- At the very end of each entry method, the following code should be written
        -- if nextCount > 0 
        --    nextSem.Up ()
        -- else
        --    monitorLock.Up ()

    -- Normal threads waiting for a condition will wait on a semaphore called "sem".
    -- Threads that have signaled for a condition will wait on a (global) semaphore "nextSem".
...
/*
  ...
  behavior HoareCondition
      ...
      method Wait (monitorLock: ptr to Semaphore)
      ...
      method Signal (monitorLock: ptr to Semaphore)
      -- See if anybody is waiting for this condition
      -- If someone is waiting for it:
        -- Wake it up
        -- Wait on nextSem which is a high priority waiting list
        if cnt > 0
      ...
      method Broadcast (monitorLock: ptr to Semaphore)
        var i: int

        nextCount = nextCount + 1
        for i = 0 to cnt -1 by 1
          sem.Up()
          endFor
        nextSem.Down ()
      nextCount = nextCount - 1
  ...
  behavior ThreadManager
      ...
      method Init ()
        ...
        -- 
          ...
          -- Initialize the threads in threadTable and add them to freeList
      ...
      method GetANewThread () returns ptr to Thread
        ...
        -- 
        var threadPointer: ptr to Thread
        -- First, obtain monitors lock
        ...
        -- Obtain a free thread, change its status and return it
        threadPointer = freeList.Remove()
        -- -> ?
        threadPointer.status = JUST_CREATED
        
        -- unlock monitors mutex
  ...
  behavior ProcessManager
      ...
      method Init ()
        ...
        --
          ...
          -- Initialize the PCBs in processTable and add them to freeList
          for i = 0 to MAX_NUMBER_OF_PROCESSES - 1 by 1
            processTable[i].Init()
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
      ...
      method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
          ...
          -- Wait until enough frames become available
          while numberFreeFrames < numFramesNeeded
              newFramesAvailable.Wait(&frameManagerLock)
            endWhile

          -- Find free frames and add them to aPageTable
          for i = 0 to numFramesNeeded - 1 by 1
              f = framesInUse.FindZeroAndSet ()
              frameAddr = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + (f * PAGE_SIZE)
      ...
      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
          ...
          frameManagerLock.Lock()
          -- Find frames that are in aPageTable and release them by resetting their respective bits in framesInUse to 0.
          for i = 0 to aPageTable.numberOfPages - 1 by 1
...
-----------------------------  Handle_Sys_Join  ---------------------------------
  ...
  function Handle_Sys_Join (processID: int) returns int
      -- NOT IMPLEMENTED
      print ("Handle_Sys_Join: processID is ")
      printInt (processID)
      nl ()
      return 2000
    endFunction
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
      ...
      ---- Code for task 2: printing the arguments
      length = currentThread.myProcess.addrSpace.GetStringFromVirtual (& strBuffer, filename asInteger, MAX_STRING_SIZE)
      print ("Handle_Sys_Exec: filename is ")
      print (& strBuffer)
      nl ()
      ---------------------

      -- Check if loading filename to kernel was successful
      if length < 0
        return -1
      endIf

      -- open the file whose name is specified in kernelAddr
      ...
      -- check if loading the program was successful
      if initPC < 0
        return -1
      endIf

      -- free all the frames in the previous address space and replace it with the new address space
...
-----------------------------  Handle_Sys_Create  ---------------------------------
  ...
  function Handle_Sys_Create (filename: ptr to array of char) returns int
      ...
      length = currentThread.myProcess.addrSpace.GetStringFromVirtual (& kernelAddr, filename asInteger, MAX_STRING_SIZE)
